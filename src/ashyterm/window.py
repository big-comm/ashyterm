...
# Lazy initialization for AI prompts - avoids translating 140+ strings at import time
_cached_ai_prompts: Optional[List[str]] = None


def _get_ai_prompts() -> List[str]:
    """Get AI prompts list with lazy translation."""
    global _cached_ai_prompts
    if _cached_ai_prompts is None:
        _cached_ai_prompts = [
            _("How can I monitor CPU and memory usage in real time?"),
            _("Which commands show the processes consuming the most memory?"),
            _("Why is my disk stuck at 100% usage and how do I investigate it?"),
            _("How can I locate large files taking up space under /?"),
            _("What steps help troubleshoot a slow SSH connection?"),
            _("How can I automate daily incremental backups with rsync?"),
            _("How do I remove orphan packages and clean the cache on BigLinux?"),
            _("How can I list services that are enabled and active on systemd?"),
            _("What should I examine when boot time is too long?"),
            _("How do I monitor temperature and fan speeds from the terminal?"),
            _("How can I configure alerts when CPU usage exceeds a threshold?"),
            _("How do I create a script that restarts a service automatically?"),
            _("How do I automatically mount a remote NFS share?"),
            _("How can I debug network issues using mtr and traceroute?"),
            _("What best practices should I follow to harden SSH?"),
            _("How do I schedule updates with pacman and log the results?"),
            _("How do I detect and terminate zombie processes?"),
            _("How do I compare two directories to find differences?"),
            _("How can I configure automatic backups of /etc with git?"),
            _("How do I troubleshoot high disk I/O usage in databases?"),
            _("How can I check available kernel versions and update safely?"),
            _("What is the best way to install proprietary GPU drivers?"),
            _("How do I configure multiple monitors on X11 and Wayland?"),
            _("How can I adjust display brightness from the terminal?"),
            _("How do I find which process is listening on a TCP port?"),
            _("How can I release an occupied port without rebooting?"),
            _("How do I identify and kill stuck SSH sessions?"),
            _("How can I configure an SSH tunnel for secure remote access?"),
            _("How do I sync folders using rsync with compression?"),
            _("How do I schedule recurring tasks with systemd timers?"),
            _("How can I configure custom log rotation with logrotate?"),
            _("How do I access the systemd journal from another host?"),
            _("How do I tail logs in real time and filter by keyword?"),
            _("How can I check which package installed a specific file?"),
            _("How do I list package dependencies before installing?"),
            _("How can I simulate a pacman upgrade without changing the system?"),
            _("How do I recover a system that fails to boot after a kernel update?"),
            _("How do I mount ISO images without burning them?"),
            _("How do I verify downloads using sha256sum?"),
            _("How can I set persistent aliases for frequent commands?"),
            _("How do I create custom shell functions in my bashrc?"),
            _("How do I share a folder with Samba and manage permissions?"),
            _("How do I mount a CIFS share securely with credentials?"),
            _("How do I find my internal and external IP address quickly?"),
            _("How can I test network speed from the command line?"),
            _("How do I scan local devices with nmap?"),
            _("How can I troubleshoot slow or failing DNS?"),
            _("How do I configure a local DNS resolver with systemd-resolved?"),
            _("How can I capture suspicious traffic with tcpdump?"),
            _("How do I limit bandwidth for a process using tc or trickle?"),
            _("How can I inspect routing tables and metrics?"),
            _("How do I configure NTP servers and sync time automatically?"),
            _("How do I manage users and groups with useradd and gpasswd?"),
            _("How do I reset the root password if I forget it?"),
            _("How can I configure sudo without granting full root access?"),
            _("How do I audit successful and failed logins on the system?"),
            _("How can I temporarily lock an account after failed attempts?"),
            _("How do I configure SSH keys and disable password authentication?"),
            _("How can I use fail2ban to protect exposed services?"),
            _("How do I enable the firewall with ufw and create basic rules?"),
            _("How can I inspect and adjust rules with nftables?"),
            _("How do I detect altered files using tripwire or aide?"),
            _("How do I configure encrypted backups automatically with borg?"),
            _("How can I sync backups to the cloud using rclone?"),
            _("How do I set up software RAID and monitor its health?"),
            _("How can I recover data from a degraded RAID array?"),
            _("How do I analyse SMART attributes to predict disk failures?"),
            _("How can I expand an LVM partition without data loss?"),
            _("How do I move the home directory to another partition?"),
            _("How do I create a swapfile and tune swappiness?"),
            _("How can I clear cached memory without rebooting?"),
            _("How do I reduce power consumption on Linux laptops?"),
            _("How can I troubleshoot frequent GNOME or KDE freezes?"),
            _("How do I reset desktop environment settings from the terminal?"),
            _("How do I install additional fonts and set them system-wide?"),
            _("How can I change keyboard layout permanently?"),
            _("How do I enable network printing support with CUPS?"),
            _("How can I troubleshoot PulseAudio or PipeWire sound issues?"),
            _("How do I detect and configure USB devices automatically?"),
            _("How can I set up a simple web server with nginx?"),
            _("How do I secure Apache with HTTPS using Let's Encrypt?"),
            _("How can I optimise PHP-FPM for high traffic?"),
            _("How do I configure a reverse proxy with caching?"),
            _("How can I monitor Docker containers and restart them automatically?"),
            _("How do I inspect container logs and limit resource usage?"),
            _("How do I build optimised multi-stage Docker images?"),
            _("How can I manage pods and deployments in a local Kubernetes cluster?"),
            _("How do I set up a basic CI/CD pipeline with GitLab Runner?"),
            _("How can I create isolated Python environments with venv?"),
            _("How do I build custom deb or rpm packages?"),
            _("How can I compile a custom kernel and install it?"),
            _("How do I measure application performance with perf and flame graphs?"),
            _("How can I use strace and ltrace to debug processes?"),
            _("How do I investigate deadlocks with gdb and core dumps?"),
            _("How can I enable SELinux or AppArmor and create simple policies?"),
            _(
                "How do I validate security policies before applying them in production?"
            ),
            _("How can I configure site-to-site OpenVPN?"),
            _("How do I set up a WireGuard server with multiple users?"),
            _("How can I detect and mitigate excessive swap usage?"),
            _("How do I track why a process was killed by the OOM killer?"),
            _("How can I configure disk quotas for users and groups?"),
            _("How do I automate cleanup of /tmp and temporary directories?"),
            _("How can I refresh pacman mirrors and clean the cache?"),
            _("How do I share clipboard contents and files via sshfs?"),
            _("How can I configure email notifications for critical failures?"),
            _("How do I collect metrics with Prometheus and basic exporters?"),
            _("How can I build Grafana dashboards for Linux servers?"),
            _("How do I apply security patches automatically?"),
            _("How can I keep Ansible playbooks organised for multiple hosts?"),
            _("How do I troubleshoot slow PostgreSQL databases?"),
            _("How can I diagnose memory usage in Java applications?"),
            _("How do I integrate LDAP authentication into local services?"),
            _("How do I uninstall a program?"),
            _("How do I update my system and every application at once?"),
            _("How can I connect to a Wi-Fi network?"),
            _("How do I set up my printer?"),
            _("How can I take a screenshot of the screen?"),
            _("How do I connect a second monitor?"),
            _("What should I do if there is no sound?"),
            _("How can I change the screen resolution?"),
            _("How do I change the wallpaper and system theme?"),
            _("How can I mount and unmount a USB drive or external disk?"),
            _("How do I format a USB stick (FAT32, NTFS, ext4)?"),
            _("How do I install new software packages?"),
            _("How do I remove installed software packages?"),
            _("How do I check how much free disk space I have?"),
            _("How do I find which files are taking the most space?"),
            _("How can I search for a specific file or text?"),
            _("How do I check open ports on my machine?"),
            _("How can I view running processes and resource usage?"),
            _("How do I check recent system logs?"),
            _("How do I update the kernel safely?"),
            _("How can I enable automatic updates?"),
            _("How do I create and manage new users?"),
            _("How do I change file permissions safely?"),
            _("How can I compress or extract archive files?"),
            _("How do I verify that my network connection is working?"),
            _("How do I configure proxy settings for the system?"),
            _("How can I synchronise system time with online sources?"),
            _("How do I view detailed hardware information?"),
            _("How can I install or update device drivers?"),
            _("How do I manage startup applications?"),
            _("How can I disable unwanted services?"),
            _("How do I schedule tasks with cron?"),
            _("How can I secure SSH on a new server?"),
            _("How do I monitor system logs in real time?"),
            _("How can I back up my home directory automatically?"),
            _("How do I restore a backup safely?"),
            _("How can I encrypt files or folders?"),
            _("How do I set up remote desktop access?"),
            _("How can I configure a host-based firewall?"),
            _("How do I whitelist or blacklist IP addresses?"),
            _("How can I check disk health and SMART status?"),
            _("How do I monitor I/O usage per process?"),
            _("How can I check current RAM usage?"),
            _("How do I clear the DNS cache?"),
            _("How can I troubleshoot boot issues quickly?"),
            _("How do I recover deleted files?"),
            _("How can I set environment variables permanently?"),
            _("How do I install development tools and headers?"),
            _("How can I manage Python packages with pip?"),
            _("How do I manage Node.js packages with npm?"),
            _("How can I install Docker on my system?"),
            _("How do I remove Docker completely?"),
            _("How can I manage Docker images efficiently?"),
            _("How do I manage Docker volumes safely?"),
            _("How can I inspect Docker networks?"),
            _("How do I bootstrap a Kubernetes cluster?"),
            _("How can I install and configure a SQL database?"),
            _("How do I secure a database server?"),
            _("How can I monitor database performance metrics?"),
            _("How do I back up a database automatically?"),
            _("How can I restore a database backup?"),
            _("How do I upgrade system packages safely?"),
            _("How can I check for package updates without installing them?"),
            _("How do I set up a local repository mirror?"),
            _("How can I troubleshoot package conflicts?"),
            _("How do I configure proxy settings for package managers?"),
            _("How can I clean the package cache safely?"),
            _("How do I manage Flatpak or Snap packages?"),
            _("How can I monitor resource usage per container?"),
            _("How do I export container metrics to Prometheus?"),
            _("How can I trigger alerts when CPU exceeds 70%?"),
            _("How do I archive system logs for compliance?"),
            _("How can I set up centralised logging with Loki?"),
            _("How do I configure Grafana alerts for system metrics?"),
            _("How can I deploy applications with Helm charts?"),
            _("How do I roll back a Kubernetes deployment?"),
            _("How can I manage secrets securely on Linux?"),
            _("How do I inspect SELinux or AppArmor audit logs?"),
            _("How can I test firewall rules without disrupting traffic?"),
            _("How do I benchmark disk throughput?"),
            _("How can I track battery health on laptops?"),
            _("How do I configure automatic screen locking and idle timeouts?"),
            _("How can I share files over the network easily?"),
            _("How do I monitor GPU usage on Linux?"),
            _("How can I check for firmware updates?"),
            _("How do I enable hardware acceleration in browsers?"),
            _("How can I detect memory leaks in applications?"),
            _("How do I profile Python scripts for performance?"),
            _("How can I measure latency between servers?"),
            _("How do I configure a VPN client connection?"),
            _("How can I script repetitive maintenance tasks?"),
            _("How do I document server configuration automatically?"),
            _("How can I manage dotfiles across multiple machines?"),
            _("How do I set up passwordless sudo for automation?"),
            _("How can I monitor cron job failures?"),
            _("How do I send notifications to Slack or Matrix from scripts?"),
            _("How can I manage secrets in environment variables?"),
            _("How do I install and maintain ZFS on Linux?"),
            _("How can I create and restore Btrfs snapshots?"),
            _("How do I check ext4 filesystem health?"),
            _("How can I detect hardware errors with dmesg?"),
            _("How do I tune kernel parameters with sysctl?"),
            _("How can I configure huge pages for databases?"),
            _("How do I enable and manage cgroups v2?"),
            _("How can I collect performance data with sar?"),
            _("How do I investigate high load average values?"),
            _("How can I test backup restores automatically?"),
            _("How do I rotate SSH keys on a schedule?"),
            _("How can I enforce password policies on Linux?"),
            _("How do I audit installed packages for CVEs?"),
            _("How can I use nix-shell or direnv for reproducible dev environments?"),
            _("How do I manage multiple Python versions with pyenv?"),
            _("How can I run containers rootless with Podman?"),
            _("How do I tail Kubernetes logs with stern or kubectl?"),
            _("How can I instrument applications with OpenTelemetry?"),
            _("How do I configure single sign-on for Linux services?"),
            _("How can I export metrics from systemd services?"),
            _("How do I manage secrets with HashiCorp Vault on Linux?"),
            _("How can I secure shared desktop sessions?"),
            _("How do I offer remote assistance to another Linux user?"),
            _("How can I set up an immutable system partition?"),
            _("How do I verify package integrity with pkgfile or debsums?"),
            _("How can I preconfigure new machines with cloud-init?"),
            _("How do I use firewalld zones effectively?"),
            _("How can I configure USBGuard to control removable media?"),
            _("How do I script GUI interactions with xdotool or ydotool?"),
            _("How can I schedule notifications for maintenance windows?"),
            _("How do I run graphical apps inside containers securely?"),
            _("How can I create custom systemd targets?"),
            _("How do I instrument cron jobs with logging and alerts?"),
            _("How can I migrate user data between Linux distributions?"),
            _("How do I build automated smoke tests for servers?"),
            _("How can I monitor power consumption per process?"),
            _("How do I set up screen sharing on Wayland?"),
            _("How can I use bpftrace to inspect kernel events?"),
            _("How do I configure VPN access for contractors securely?"),
            _("How can I manage encrypted secrets in git repositories?"),
            _("How do I containerise legacy applications on Linux?"),
            _("How can I prepare a Linux workstation for developers quickly?"),
            _("How do I tune swappiness and vm.dirty ratios for workloads?"),
            _("How can I monitor NVMe SSD health?"),
            _("How do I benchmark network throughput between hosts?"),
            _("How can I secure cron and at jobs from tampering?"),
            _("How do I implement just-in-time sudo elevation?"),
            _("How can I integrate system metrics with Alertmanager?"),
            _("How do I set up maintenance mode pages on nginx?"),
            _("How can I collect audit logs required for compliance?"),
            _("How do I gracefully decommission a Linux server?"),
            _("How can I document operational runbooks efficiently?"),
        ]
    return _cached_ai_prompts
...
class CommTerminalWindow(Adw.ApplicationWindow):
    ...
    def _show_ai_prompt_dialog(self, terminal) -> None:
        ...
        suggestion_label.set_wrap_mode(Pango.WrapMode.WORD_CHAR)
        ...
        # Get AI prompts (lazy loaded)
        ai_prompts = _get_ai_prompts()
        ...
        max_prompt_length = max((len(p) for p in ai_prompts), default=0)
        approx_width = max(660, min(1100, max_prompt_length * 7 + 340))
        approx_height = 340
        if ai_prompts:
            approx_height += min(420, len(ai_prompts) * 28)
        approx_height = min(820, approx_height)
        dialog.set_default_size(int(approx_width), int(approx_height))
                ...
                if query:
                    if matches:
                        suggestion_label.set_text(
                            _("Sugestões correspondentes: {count}").format(count=matches)
                        )
                    else:
                        suggestion_label.set_text(_("Nenhuma sugestão encontrada."))
                else:
                    suggestion_label.set_text(
                        _("Digite sua pergunta ou escolha uma das sugestões.")
                    )
            if suggestions_scrolled:
                min_height = 0
                if matches:
                    min_height = min(420, max(120, matches * 30))
                else:
                    min_height = 140
