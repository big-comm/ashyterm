...
from .osc7 import OSC7Buffer, OSC7Info, OSC7Parser, format_tab_title
...
class OSC7TerminalTracker:
        ...
        try:
            with self._lock:
                if terminal in self._terminals:
                    ...
                    self.logger.debug(
                        f"Stopped OSC7 tracking for terminal: '{base_title}'"
                    )
                ...
                try:
                    ...
                    if current_uri:
                        self._handle_directory_uri_change(
                            terminal, terminal_data, current_uri
                        )
                        return
                except Exception as e:
    ...
    def _parse_recent_terminal_output(
        self, terminal: Vte.Terminal, terminal_data: Dict[str, Any]
    ) -> None:
        """
    ...
    def _handle_osc7_detected(
        ...
        try:
            ...
            GLib.idle_add(
                self._update_tab_title_safe, terminal, terminal_data, osc7_info
            )
