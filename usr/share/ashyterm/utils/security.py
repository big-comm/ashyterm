...
class InputSanitizer:
    ...
    def sanitize_path(path: str) -> str:
        ...
        sanitized = "".join(
            char for char in sanitized if ord(char) >= 32 and char != "\0"
        )
...
class PathValidator:
    ...
    def validate_file_permissions(
        file_path: str, required_permissions: Optional[int] = None
        ...
        try:
            ...
            if required_permissions is not None:
                if current_permissions != required_permissions:
                    return False, _("Incorrect permissions: {} (expected {})").format(
                        oct(current_permissions), oct(required_permissions)
                    )
        ...
        try:
            ...
            if not is_valid:
                findings.append({
                    ...
                    "message": error,
                    "recommendation": _(
                        "Set secure file permissions (600 for files, 700 for directories)"
                    ),
                })
            ...
            if hasattr(os, "getuid"):
                ...
                if file_stat.st_uid != current_uid:
                    findings.append({
                        ...
                        "message": _("File is owned by different user: {}").format(
                            file_stat.st_uid
                        ),
        ...
        try:
            ...
            if current_permissions & 0o077:  # Group or other have permissions
                findings.append({
                    ...
                    "message": _(
                        "Directory has overly permissive permissions: {}"
                    ).format(oct(current_permissions)),
