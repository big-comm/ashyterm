...
class SecureStorage:
    ...
    def _derive_key_scrypt(
        self, password: bytes, salt: bytes, n: int = 16384, r: int = 8, p: int = 1
    ) -> bytes:
    ...
    def _create_master_key(
        self, passphrase: Optional[str] = None, method: str = KeyDerivationMethod.SCRYPT
    ) -> bytes:
        """
            ...
            if passphrase:
                ...
                if method == KeyDerivationMethod.SCRYPT:
                    ...
                    master_key = self._derive_key_pbkdf2(
                        password_bytes, salt, iterations
                    )
                    config_data = {
                        "method": method,
                        "iterations": iterations,
    ...
    def _load_master_key(self, passphrase: Optional[str] = None) -> Optional[bytes]:
        ...
        try:
            if not self.config_file.exists() or not self.salt_file.exists():
                self.logger.debug(
                    "Master key files not found - first run or missing keys"
                )
                return None
            ...
            if method == "random":
                ...
                if not passphrase:
                    raise EncryptionError(
                        "key loading", "Passphrase required for derived key"
                    )
                ...
                if method == KeyDerivationMethod.SCRYPT:
                    ...
                    master_key = self._derive_key_pbkdf2(
                        password_bytes, salt, config.get("iterations", 100000)
                    )
                ...
                raise EncryptionError(
                    "key loading", f"Unknown key derivation method: {method}"
                )
    ...
    def initialize(
        self, passphrase: Optional[str] = None, force_create: bool = False
    ) -> bool:
        """
        ...
        try:
            self.logger.debug(
                f"Initializing secure storage - force_create: {force_create}"
            )
            ...
            if force_create or not has_key:
                ...
                try:
                    ...
                    if isinstance(e, InvalidToken):
                        self.logger.warning(
                            "Corrupted master key detected, regenerating..."
                        )
            ...
            self.logger.warning(
                "Failed to initialize secure storage - no master key available"
            )
            return False
    ...
    def _has_master_key(self) -> bool:
        ...
        return (
            ...
            and self.salt_file.exists()
            and (self.key_file.exists() or self._is_passphrase_derived())
        )
    ...
    def change_passphrase(
        self, old_passphrase: Optional[str], new_passphrase: Optional[str]
    ) -> bool:
        """
    ...
    def restore_keys(self, backup_path: Path) -> bool:
        ...
        try:
            ...
            backup_files = [
                backup_path / f.name
                for f in [self.key_file, self.salt_file, self.config_file]
            ]
            ...
            for backup_file, target_file in zip(
                backup_files, [self.key_file, self.salt_file, self.config_file]
            ):
