...
from ..utils.crypto import (decrypt_password, encrypt_password,
                            get_secure_storage, is_encryption_available)
...
from ..utils.security import (HostnameValidator, InputSanitizer,
                              SSHKeyValidator, sanitize_folder_name,
...
class SessionItem(GObject.GObject):
    ...
    def __init__(
        ...
        self.logger.debug(
            f"Session item created: '{self.name}' (type: {self.session_type})"
        )
        ...
        try:
            ...
            if value not in valid_types:
                raise SessionValidationError(
                    ...
                    [
                        _(
                            "Invalid session type: {type}. Must be one of: {valid_types}"
                        ).format(type=value, valid_types=", ".join(valid_types))
                    ],
                )
            ...
            if value:
                ...
                if not HostnameValidator.is_valid_hostname(sanitized):
                    raise SessionValidationError(
                        self.name,
                        [
                            _("Invalid hostname format: {hostname}").format(
                                hostname=value
                            )
                        ],
                    )
        ...
        try:
            ...
            if value not in valid_types:
                raise SessionValidationError(
                    ...
                    [
                        _(
                            "Invalid authentication type: {type}. Must be one of: {valid_types}"
                        ).format(type=value, valid_types=", ".join(valid_types))
                    ],
                )
    ...
    def auth_value(self) -> str:
        ...
        if self.uses_password_auth() and self._auth_value:
            try:
                ...
                self.logger.error(
                    f"Password decryption failed for session '{self.name}': {e}"
                )
                return self._auth_value
            ...
            if self.uses_password_auth() and value:
                ...
                if is_encryption_available():
                    ...
                    if storage.is_initialized():
                        try:
                            ...
                            self.logger.debug(
                                f"Password encrypted for session '{self.name}'"
                            )
                        except Exception as e:
                            self.logger.warning(
                                f"Password encryption failed for session '{self.name}': {e}"
                            )
                            self._auth_value = value
                    else:
                        self.logger.warning(
                            f"Encryption not initialized, storing password as plain text for session '{self.name}'"
                        )
                        self._auth_value = value
                else:
                    self.logger.warning(
                        f"Encryption not available, storing password as plain text for session '{self.name}'"
                    )
                    self._auth_value = value
            else:
                ...
                if self.uses_key_auth() and value:
                    ...
                    try:
                        key_path = Path(value)
                        if key_path.exists():
                            is_valid, error = SSHKeyValidator.validate_ssh_key_path(
                                value
                            )
                            if not is_valid:
                                self.logger.warning(
                                    f"SSH key validation failed for session '{self.name}': {error}"
                                )
                    except Exception as e:
    ...
    def port(self, value: int) -> None:
        ...
        try:
            ...
            if not (1 <= port_val <= 65535):
                raise SessionValidationError(
                    self.name, [_("Port must be between 1 and 65535")]
                )
    ...
    def mark_used(self) -> None:
        ...
        try:
            ...
            self._use_count += 1
            self.logger.debug(
                f"Session '{self.name}' marked as used (count: {self._use_count})"
            )
    ...
    def validate(self) -> bool:
        ...
        try:
            ...
            if self.is_ssh():
                if not self.host:
                    self._validation_errors.append(
                        _("Host is required for SSH sessions")
                    )
                ...
                if self.uses_key_auth() and self.auth_value:
                    ...
                    try:
                        ...
                        if not is_valid:
                            self._validation_errors.append(
                                _("SSH key validation failed: {error}").format(
                                    error=error
                                )
                            )
                    except Exception as e:
                        self._validation_errors.append(
                            _("SSH key validation error: {error}").format(error=str(e))
                        )

                if not self.auth_type:
                    self._validation_errors.append(
                        _("Authentication type is required for SSH sessions")
                    )
            ...
            if self._validation_errors:
                self.logger.warning(
                    f"Session validation failed for '{self.name}': {self._validation_errors}"
                )
                return False
            ...
            self._validation_errors.append(
                _("Validation error: {error}").format(error=str(e))
            )
            return False
        ...
        try:
            ...
            if self.parent_path and self.path:
                if not self.path.startswith(self.parent_path + "/"):
                    self._validation_errors.append(
                        _("Path is not consistent with parent path")
                    )
            ...
            if self._validation_errors:
                self.logger.warning(
                    f"Folder validation failed for '{self.name}': {self._validation_errors}"
                )
                return False
