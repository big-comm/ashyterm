...
from ..utils.security import (create_security_auditor, sanitize_folder_name,
                              sanitize_session_name, validate_session_data)
...
class SessionOperations:
    ...
    def add_session(
        ...
        with self._operation_lock:
            try:
                self.logger.debug(
                    f"Adding session: '{session.name}' (type: {session.session_type})"
                )

                # Validate session
                validation_result = self._validate_session_for_add(
                    session, validate_security
                )
                ...
                if duplicate_check:
                    ...
                    return OperationResult(
                        False,
                        _(
                            "Session '{name}' already exists in the specified folder"
                ...
                if original_name != session.name:
                    result.add_warning(
                        _("Session name sanitized: '{original}' -> '{new}'").format(
                            original=original_name, new=session.name
                        )
                    )
                ...
                log_session_event(
                    "added",
                    session.name,
                    f"type: {session.session_type}, folder: {session.folder_path}",
                )
                ...
                log_error_with_context(
                    e, f"add session {session.name}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to add session: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result

    def _validate_session_for_add(
        self, session: SessionItem, validate_security: bool
    ) -> OperationResult:
        """
        ...
        try:
            ...
            if not session.validate():
                ...
                return OperationResult(
                    False,
                    _("Session validation failed: {errors}").format(
                        errors=", ".join(errors)
                    ),
                )
            ...
            if validate_security and session.is_ssh() and self.security_auditor:
                try:
                    ...
                    if not is_valid:
                        return OperationResult(
                            False,
                            _("Security validation failed: {errors}").format(
                    ...
                    for finding in findings:
                        if finding["severity"] in ["medium", "high"]:
                            ...
                            return OperationResult(
                                False,
                                _("Critical security issue: {message}").format(
                                    message=finding["message"]
                                ),
                            )
                    ...
                    result.add_warning(
                        _("Security validation error: {error}").format(error=str(e))
                    )
            ...
            if session.folder_path:
                if not self._folder_exists(session.folder_path):
                    return OperationResult(
                        False,
                        _("Target folder '{folder}' does not exist").format(
            ...
            return OperationResult(
                False, _("Validation error: {error}").format(error=str(e))
            )

    def update_session(
        self,
        position: int,
        updated_session: SessionItem,
        validate_security: bool = True,
        create_backup: bool = True,
    ) -> OperationResult:
        """
        ...
        with self._operation_lock:
            try:
                if not (0 <= position < self.session_store.get_n_items()):
                    return OperationResult(
                        False,
                        _("Invalid position: {position}").format(position=position),
                    )
                ...
                if not isinstance(original_session, SessionItem):
                    return OperationResult(
                        False,
                        _("Item at position {position} is not a session").format(
                            position=position
                        ),
                    )
                ...
                self.logger.debug(
                    f"Updating session: '{original_name}' -> '{updated_session.name}'"
                )
                ...
                if updated_session.name != original_name:
                    duplicate_check = self._check_session_duplicate(
                        updated_session, exclude_session=original_session
                    )
                    if duplicate_check:
                        self._stats["validation_failures"] += 1
                        return OperationResult(
                            False,
                            _(
                                "Session name '{name}' already exists in the target folder"
                            ).format(name=updated_session.name),
                        )
                ...
                result = self._update_session_properties(
                    original_session, updated_session
                )
                ...
                if original_updated_name != updated_session.name:
                    result.add_warning(
                        _("Session name sanitized: '{original}' -> '{new}'").format(
                            original=original_updated_name, new=updated_session.name
                        )
                    )
                ...
                log_session_event(
                    "updated", updated_session.name, f"from: {original_name}"
                )

                self.logger.info(
                    f"Session updated successfully: '{original_name}' -> '{updated_session.name}'"
                )
                result.message = _("Session '{name}' updated successfully").format(
                ...
                self._stats["operation_errors"] += 1
                self.logger.error(
                    f"Failed to update session at position {position}: {e}"
                )
                log_error_with_context(
                    e, f"update session at {position}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to update session: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result

    def _validate_session_for_update(
        self, original: SessionItem, updated: SessionItem, validate_security: bool
    ) -> OperationResult:
        """
        ...
        try:
            ...
            if not updated.validate():
                ...
                return OperationResult(
                    False,
                    _("Updated session validation failed: {errors}").format(
                        errors=", ".join(errors)
                    ),
                )
            ...
            if validate_security and updated.is_ssh() and self.security_auditor:
                try:
                    ...
                    if (
                        ...
                        result.add_warning(
                            _(
                                "Security score decreased: {original} -> {updated}"
                            ).format(original=original_score, updated=updated_score)
                        )
            ...
            return OperationResult(
                False, _("Update validation error: {error}").format(error=str(e))
            )

    def _update_session_properties(
        self, original: SessionItem, updated: SessionItem
    ) -> OperationResult:
        """
        ...
        try:
            ...
            return OperationResult(
                False, _("Property update failed: {error}").format(error=str(e))
            )

    def remove_session(
        self, session: SessionItem, create_backup: bool = True
    ) -> OperationResult:
        """
        ...
        with self._operation_lock:
            try:
                ...
                if position == -1:
                    return OperationResult(
                        False,
                        _("Session '{name}' not found in store").format(
                            name=session_name
                        ),
                    )
                ...
                log_session_event(
                    "removed", session_name, f"type: {session.session_type}"
                )
                ...
                result = OperationResult(
                    True,
                    _("Session '{name}' removed successfully").format(
                        name=session_name
                    ),
                )
                ...
                log_error_with_context(
                    e, f"remove session {session.name}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to remove session: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result
        ...
        with self._operation_lock:
            try:
                ...
                if custom_name:
                    ...
                    duplicate.name = generate_unique_name(
                        _("Copy of {name}").format(name=session.name), existing_names
                    )
                ...
                if not duplicate.validate():
                    ...
                    return OperationResult(
                        False,
                        _("Duplicate validation failed: {errors}").format(
                            errors=", ".join(errors)
                        ),
                    )
                ...
                if duplicate_check:
                    ...
                    return OperationResult(
                        False,
                        _("Duplicate name '{name}' already exists").format(
                            name=duplicate.name
                        ),
                    )

                # Add duplicate
                add_result = self.add_session(
                    duplicate, validate_security=False, create_backup=create_backup
                )
                ...
                log_session_event(
                    "duplicated",
                    f"{original_name} -> {duplicate.name}",
                    "session duplication",
                )

                self.logger.info(
                    f"Session duplicated successfully: '{original_name}' -> '{duplicate.name}'"
                )

                result = OperationResult(
                    True,
                    _("Session duplicated as '{name}'").format(name=duplicate.name),
                    duplicate,
                )
    ...
    def move_session_to_folder(
        self, session: SessionItem, target_folder_path: str, create_backup: bool = True
    ) -> OperationResult:
        """
        ...
        with self._operation_lock:
            try:
                ...
                self.logger.debug(
                    f"Moving session '{session_name}': '{original_folder}' -> '{target_folder_path}'"
                )
                ...
                if session.name in existing_names:
                    ...
                    result = OperationResult(
                        True,
                        _("Session moved and renamed to '{name}'").format(
                            name=new_name
                        ),
                        session,
                    )
                    result.add_warning(
                        _(
                            "Session renamed to avoid conflict: '{original}' -> '{new}'"
                        ).format(original=session_name, new=new_name)
                    )
                else:
                    target_display = target_folder_path or _("root")
                    result = OperationResult(
                        True,
                        _("Session moved to '{folder}'").format(folder=target_display),
                        session,
                    )
                ...
                if not self._save_changes():
                    ...
                    return OperationResult(
                        False, _("Failed to save after moving session")
                    )
                ...
                log_session_event(
                    "moved",
                    session.name,
                    f"from: {original_folder}, to: {target_folder_path}",
                )

                target_display = target_folder_path or _("root")
                self.logger.info(
                    f"Session moved successfully: '{session_name}' to '{target_display}'"
                )
                ...
                log_error_with_context(
                    e, f"move session {session.name}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to move session: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result

    def add_folder(
        self, folder: SessionFolder, create_backup: bool = True
    ) -> OperationResult:
        """
        ...
        with self._operation_lock:
            try:
                self.logger.debug(
                    f"Adding folder: '{folder.name}' (path: {folder.path})"
                )
                ...
                if not folder.validate():
                    ...
                    return OperationResult(
                        False,
                        _("Folder validation failed: {errors}").format(
                            errors=", ".join(errors)
                        ),
                    )
                ...
                if self._folder_path_exists(folder.path):
                    ...
                    return OperationResult(
                        False,
                        _("Folder path '{path}' already exists").format(
                            path=folder.path
                        ),
                    )
                ...
                self.logger.info(
                    f"Folder added successfully: '{folder.name}' (path: {folder.path})"
                )
                ...
                if original_name != folder.name:
                    result.add_warning(
                        _("Folder name sanitized: '{original}' -> '{new}'").format(
                            original=original_name, new=folder.name
                        )
                    )
                ...
                log_error_with_context(
                    e, f"add folder {folder.name}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to add folder: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result

    def update_folder(
        self, position: int, updated_folder: SessionFolder, create_backup: bool = True
    ) -> OperationResult:
        """
        ...
        with self._operation_lock:
            try:
                ...
                if not isinstance(original_folder, SessionFolder):
                    return OperationResult(
                        False,
                        _("Item at position {position} is not a folder").format(
                            position=position
                        ),
                    )
                ...
                self.logger.debug(
                    f"Updating folder: '{old_name}' -> '{updated_folder.name}'"
                )
                ...
                if not updated_folder.validate():
                    ...
                    return OperationResult(
                        False,
                        _("Updated folder validation failed: {errors}").format(
                            errors=", ".join(errors)
                        ),
                    )

                # Check path conflicts (excluding current folder)
                if updated_folder.path != old_path and self._folder_path_exists(
                    updated_folder.path, exclude_folder=original_folder
                ):
                    self._stats["validation_failures"] += 1
                    return OperationResult(
                        False,
                ...
                log_session_event(
                    "folder_updated",
                    updated_folder.name,
                    f"from: {old_name}, path: {updated_folder.path}",
                )

                self.logger.info(
                    f"Folder updated successfully: '{old_name}' -> '{updated_folder.name}'"
                )
                ...
                if original_updated_name != updated_folder.name:
                    result.add_warning(
                        _("Folder name sanitized: '{original}' -> '{new}'").format(
                            original=original_updated_name, new=updated_folder.name
                        )
                    )
                ...
                self._stats["operation_errors"] += 1
                self.logger.error(
                    f"Failed to update folder at position {position}: {e}"
                )
                log_error_with_context(
                    e, f"update folder at {position}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to update folder: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result

    def remove_folder(
        self, folder: SessionFolder, force: bool = False, create_backup: bool = True
    ) -> OperationResult:
        """
        ...
        with self._operation_lock:
            try:
                ...
                self.logger.debug(
                    f"Removing folder: '{folder_name}' (path: {folder_path}, force: {force})"
                )
                ...
                if not force and self._folder_has_children(folder_path):
                    return OperationResult(
                        False,
                        _(
                            "Cannot remove folder '{name}' - it contains sessions or subfolders. Use force removal to delete anyway."
                ...
                if position == -1:
                    return OperationResult(
                        False,
                        _("Folder '{name}' not found in store").format(
                            name=folder_name
                        ),
                    )
                ...
                if not self._save_changes():
                    ...
                    return OperationResult(
                        False, _("Failed to save after folder removal")
                    )
                ...
                log_session_event(
                    "folder_removed",
                    folder_name,
                    f"path: {folder_path}, children: {len(removed_children)}",
                )

                self.logger.info(
                    f"Folder removed successfully: '{folder_name}' (children: {len(removed_children)})"
                )
                ...
                if removed_children:
                    result.add_warning(
                        _("Removed {count} child items").format(
                            count=len(removed_children)
                        )
                    )
                ...
                log_error_with_context(
                    e, f"remove folder {folder.name}", "ashyterm.sessions.operations"
                )

                error_result = OperationResult(
                    False, _("Failed to remove folder: {error}").format(error=str(e))
                )
                error_result.metadata["exception"] = str(e)
                return error_result
    ...
    def _update_child_paths(self, old_path: str, new_path: str) -> None:
        ...
        try:
            ...
            for i in range(self.session_store.get_n_items()):
                ...
                if isinstance(session, SessionItem):
                    if session.folder_path == old_path:
                        ...
                        updated_sessions += 1
                    elif session.folder_path and session.folder_path.startswith(
                        old_path + "/"
                    ):
                        session.folder_path = session.folder_path.replace(
                            old_path + "/", new_path + "/", 1
                        )
                        updated_sessions += 1
            ...
            for i in range(self.folder_store.get_n_items()):
                ...
                if isinstance(folder, SessionFolder):
                    if folder.parent_path == old_path:
                        ...
                        updated_folders += 1
                    elif folder.parent_path and folder.parent_path.startswith(
                        old_path + "/"
                    ):
                        folder.parent_path = folder.parent_path.replace(
                            old_path + "/", new_path + "/", 1
                        )
                        folder.path = normalize_path(
            ...
            self.logger.debug(
                f"Updated child paths: {updated_sessions} sessions, {updated_folders} folders"
            )
    ...
    def _remove_folder_children(self, folder_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(self.session_store.get_n_items() - 1, -1, -1):
                session = self.session_store.get_item(i)
                if (
                    isinstance(session, SessionItem)
                    and session.folder_path == folder_path
                ):
            ...
            for i in range(self.folder_store.get_n_items() - 1, -1, -1):
                folder = self.folder_store.get_item(i)
                if (
                    isinstance(folder, SessionFolder)
                    and folder.parent_path == folder_path
                ):
    ...
    def _check_session_duplicate(
        self, session: SessionItem, exclude_session: Optional[SessionItem] = None
        ...
        try:
            for i in range(self.session_store.get_n_items()):
                ...
                if (
                    isinstance(existing_session, SessionItem)
                    and existing_session != exclude_session
                    and existing_session.name == session.name
    ...
    def _folder_path_exists(
        self, path: str, exclude_folder: Optional[SessionFolder] = None
    ) -> bool:
        """
    ...
    def _save_changes(self) -> bool:
        ...
        def save_task():
            ...
            try:
                ...
                log_error_with_context(
                    e, "async session save", "ashyterm.sessions.operations"
                )
                ...
                if isinstance(session, SessionItem):
                    if session.validate():
                        ...
                        if session.is_ssh() and self.security_auditor:
                            try:
                                findings = self.security_auditor.audit_ssh_session(
                                    session.to_dict()
                                )
                                high_severity = [
                                    f
                                ...
                                self.logger.warning(
                                    f"Security audit failed for {session.name}: {e}"
                                )
                    else:
                        invalid_sessions += 1
                        errors = session.get_validation_errors()
