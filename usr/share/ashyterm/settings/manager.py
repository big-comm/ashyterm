...
class SettingsManager:
    ...
    def _load_settings_safe(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "settings" in data and "metadata" in data:
                ...
                self.logger.debug(
                    f"Loaded settings with metadata (version: {self._metadata.version})"
                )
            else:
                ...
                if key not in self._settings:
                    ...
                    for sub_key, sub_default in default_value.items():
                        if sub_key not in self._settings[key]:
                            ...
                            updated = True
                            self.logger.debug(
                                f"Added missing default subsetting: {key}.{sub_key}"
                            )
    ...
    def save_settings(self, force: bool = False) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                self._dirty = (
                    False  # Mark as not dirty immediately to prevent multiple saves
                )
            except Exception as e:
    ...
    def _create_pre_save_backup(self):
        ...
        try:
            backup_id = self.backup_manager.create_backup(
                [self.settings_file], BackupType.AUTOMATIC, "Pre-save settings backup"
            )
    ...
    def apply_terminal_settings(self, terminal, window) -> None:
        ...
        try:
            ...
            cursor_color_str = color_scheme.get(
                "cursor", color_scheme.get("foreground", "#FFFFFF")
            )
            ...
            log_error_with_context(
                e, "terminal settings application", "ashyterm.settings"
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "settings_file_size": self.settings_file.stat().st_size
